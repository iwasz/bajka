...
void FluidSystem::Initialize ( int mode, int total )
{
	...
	// Wyczyść bufory (wektor GeomX::mBuf).
	FreeBuffers ();

	/*
	 * Dodaj jeden bufor do GeomX::mBuf. Bufor jest tylko 1 i jest typu fluid (ciecz, sztuk 1?).
	 * Jednakowoż total == ilość cząstek, więc rezerwowane jest mijesce na te cząstki w buforze.
	 */
	AddBuffer ( BFLUID, sizeof ( Fluid ), total );

	// Ustawia temu naszemu jedynemu płynowi atrybuty jak poniżej (czy może rezerwuje dla miejsce dla nich).
...
{
        // Resetuje ten nasz bufor z płynem.
	ResetBuffer ( 0, nmax );
