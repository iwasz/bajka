...
{
	if (p->attrHead > 0) {
                memset (&p->attr[p->attrHead], 0, sizeof(struct SVGAttrib));
		p->attrHead--;
	}
}
...
static void svgCreatePath(struct SVGParser* p, char closed)
{
	...
	path->strokeWidth = attr->strokeWidth * t[0];
        ...
        attr->id = &path->id;
        printf ("svgCreatePath id=%s, attr=%p, ptrId=%p\n", *attr->id, attr, *attr->id);
...
static int svgParseAttr(struct SVGParser* p, const char* name, const char* value)
{
	...
	else if (strcmp(name, "stroke-opacity") == 0)
	{
        ...
        {
                if (attr->id) {
                        printf ("svgParseAttr id=%s, attr=%p, ptrId=%p\n", value, attr, *attr->id);
                        size_t len = strlen (value) + 1;
                        *attr->id = (char *)malloc (len);
                        memcpy (*attr->id, value, len);
                }
                else {
                        printf ("svgParseAttr id=%s, attr=%p\n", value, attr);
                }
        }
	else
	{
